...
| 13 | テスト補強 | Stripe Webhookとアクセス制御のテストを追加 | `*.test.ts` | ✅ 完了 |

## 補足

- P0〜P2はすべて完了。テストは13ファイル80件。
- 未着手として残しているもの: 2FAの実装、D1バックアップ運用の整備、
  法的文書の専門家レビュー、`ENABLE_DEV_BYPASS` が /admin・/dashboard で
  効かない問題（認証を弱める変更のため要承認）。

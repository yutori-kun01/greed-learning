...
export default async function MemberPostsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'PAID': return <span className="badge" style={{ background: '#d9b45b', color: '#23180a' }}>有料記事</span>;
      default: return null;
    }
  };
            ...
            <article className="card" style={{ height: '100%', cursor: 'pointer' }}>
              ...
              <div className="card-body">
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: 12, color: 'var(--muted)', marginTop: 'auto' }}>
                  ...
                  {post.status === 'PAID' && (
                    <span style={{ color: '#d9b45b', fontWeight: 600 }}>¥{post.price?.toLocaleString()}</span>
                  )}
                </div>
              </div>
            </article>
